#' This function affine transforms images to create template for each
#' group or species.
#'
#' @name affTr
#'
...
#'
#' @details This function affine-transforms raw .jpg images to a
#'          template for each group/species using utilities of
#'          R package "RNiftyReg". These transformed images
...
#' @param img_ID A matrix containing the names of group identifiers
#'               in the first column and the sequence number of the
#'               image in that group to be affine transformed.
#' @param target Name of the target group or species name (should
#'               be one of the names in the first column of img_ID)
...
affTr <- function(img_path = NULL, # nolint
img_ID = NULL, # nolint
target = NULL,
out_path = NULL) {
...
a2 <- c(a2, ak)
  ...
  for (i in seq_len(length(a4))){
    source <- jpeg::readJPEG(paste0(img_path, a4[i]))
    result <- RNiftyReg::niftyreg(source, target, scope = "affine",
    interpolation = 0, precision = "single", threads = 4)
