...
cleqEFA<-function(img_path=NULL, out_path=NULL){
  ...
  repeat
  {
    ...
    if(cont == "y")
      {
        grDevices::dev.copy2pdf(out.type = "pdf", file= paste0(out_path,"Contour_stack.pdf"), width = 6, height = 6)
        dev.off()
        ...
        grDevices::dev.copy2pdf(out.type = "pdf", file=paste0(out_path,"Elliptical_analysis.pdf"), width = 6, height = 6)
        dev.off()
